...
static void __maybe_unused dump(const void *data, size_t size) {
  char ascii[17];
  size_t i, j;
  ascii[16] = '\0';
  for (i = 0; i < size; ++i) {
...
static size_t MemCallBack(void __maybe_unused *contents, size_t size,
                          size_t nmemb, void __maybe_unused *userp) {
  return size * nmemb;
}

static long int get_stcode(char *url) {
  CURL *curl;
  CURLcode result;
  long int st_code;
...
#endif
  for (size_t i = 0; i < t_info->block_size; i++) {
...
int do_scan(char *url, size_t path_cnt) {
  ...
  thread_info *t_list = calloc(thread_cnt, sizeof(thread_info));
  ...
  for (size_t i = 0; i < thread_cnt; i++) {
    pthread_join(t_list[i].t_id, NULL);
  }
  free(t_list);
  return 0;
}

void insert_url(char *url, size_t __maybe_unused entry, void *args) {
...
void insert_path(char *path, size_t entry, __maybe_unused void *args) {
  /* if you want to do antything on path string, you can do here*/
...
static void parse_buff(char *list, void *args,
...
int main(int argc, char *argv[]) {
  ...
  for (int j = 1; j < argc; j++) {
    if (!strcmp(argv[j], "--words")) {
      ...
      printf("Usage: %s [--words <wordlist>] [--sites <siteslist>] \n",
             argv[0]);
      exit(1);
    }
  }
